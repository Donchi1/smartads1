...
import { getTRC20Balance } from '@/utils/helpers'
...
function Page() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
              //api call 
      const wallatAmount = await getTRC20Balance(values.wallet)

      await setDoc(doc(db, `users/${auth.currentUser?.uid}`), {
        wallet:{
          address: {
            trc_20: values.wallet
          },
          type: "usdt-trc20",
  ...
  return (
    <>
    <NavHeader headerLeft={{ showHeaderLeft: true }} headerTitle={{ title: "USDT TRC20", showTitle: true }} />
